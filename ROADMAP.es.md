...
> **Convención de citas.** `§X.Y` se refiere siempre a **`Geminis Paper.es.md`**, que es la fuente
> de verdad. El `README.es.md` es un resumen con **numeración propia** —tiene 10 secciones y el
...
> barrido tardara minutos—. Todo en `geminis/predicado/RESULTS.es.md`.
...
escrita, da otro orden. Desarrollo en `geminis/estado/RESULTS.es.md`.
...
- **Revisión adversarial externa.** El diseño sobrevivió sólo a los ataques de quien lo escribió.
  Cuesta poco y vuelve rápido, y el `README.es.md` ya está armado para eso: termina en una lista de
  dónde pegar primero.
