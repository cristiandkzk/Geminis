...
Acá vive el código. El diseño está en `../Geminis Paper.es.md` (fuente de verdad) y
el plan de construcción en `../ROADMAP.es.md`. **`§X.Y` siempre cita el paper.**
...
| **2 · harness de replay** | la evidencia externa: bomba de dificultad, blobs, gas limit | ✅ **cerrada, 3 de 3.** Los tres corridos y escritos, con los datos verificados contra los EIPs y `config.go`. El veredicto, en `herramientas/RESULTS.es.md` |
| **3 · orden y liquidación** | doble gasto por lock, doble firma, la cola de §6.3 con `N` real | ✅ **los tres criterios pasan**, con una corrección al paper: la cola necesita **11** nodos y no 10 — y con la regla que cualquiera escribiría, ninguna cantidad alcanza. Ver `liquidacion/RESULTS.es.md` |
...
| `predicado/vm/` | **la máquina, en Rust.** El único directorio que cambia de lenguaje, y `README.es.md` dice por qué |
| `predicado/CRITERIA.es.md` | los siete criterios de la Fase 4, escritos antes de la primera línea y sin tocar después |
| `predicado/RESULTS.es.md` | qué dio: seis aprobados, uno reprobado, y las tres constantes de Geminis que movió |
...
| `herramientas/replay.py` · `replay_blobs.py` · `replay_gas.py` · `historial.py` | **Fase 2**: la regla candidata contra las seis veces que Ethereum corrió la bomba de dificultad. Cada dato lleva **de dónde salió y contra qué se verificó**. El resultado, en `herramientas/RESULTS.es.md` |
