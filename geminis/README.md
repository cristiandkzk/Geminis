...
The code lives here. The design is in `../Geminis Paper.md` (translation of
`../Geminis Paper.es.md`, which is the source of truth) and the build plan in `../ROADMAP.md`.
...
| **2 · replay harness** | the external evidence: difficulty bomb, blobs, gas limit | ✅ **closed, 3 of 3.** All three run and written up, with the data verified against the EIPs and `config.go`. The verdict is in `herramientas/RESULTS.md` |
| **3 · ordering and settlement** | double spend by lock, double signature, the queue of §6.3 with a real `N` | ✅ **all three criteria pass**, with a correction to the paper: the queue needs **11** nodes and not 10 — and with the rule anybody would write, no number is enough. See `liquidacion/RESULTS.md` |
...
| `predicado/vm/` | **the machine, in Rust.** The only directory that changes language, and `README.md` says why |
| `predicado/CRITERIA.md` | the seven criteria of Phase 4, written before the first line and untouched afterwards |
| `predicado/RESULTS.md` | what came out: six passed, one failed, and the three constants of Geminis it moved |
...
| `herramientas/replay.py` · `replay_blobs.py` · `replay_gas.py` · `historial.py` | **Phase 2**: the candidate rule against the six times Ethereum ran the difficulty bomb. Every datum carries **where it came from and what it was verified against**. The result is in `herramientas/RESULTS.md` |
