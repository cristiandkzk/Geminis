...
def calcular_h0(
    ...
    **`hash_id` es el hash del ruleset ancestro, no el de `params`.** Es lo único que se
    puede verificar desde A: Genesis A conoce su propia primitiva y nada más, y la
    verificabilidad del linaje "con un hash desde cualquier generación hacia atrás" depende
    de eso. Si `H0_B` se calculara con el hash que B *introduce*, verificar la transición
    exigiría ya confiar en la primitiva nueva. Cuando `params` agrega `hash/sha3-256`, el
    checkpoint que la introduce todavía sale de BLAKE2s; el siguiente ya sale de SHA-3.
    """
    return huella(
        {
...
def verificar(
    ...
    `hash_id` es el hash del ruleset A (ver `calcular_h0`). El default es el de Genesis, que es
    lo correcto para la primera transición de toda cadena.
    """
...
def motivo_linaje_invalido(
    ...
    # El hash con que se calculó cada checkpoint es el del ancestro: se arranca en el de
    # Genesis y se avanza con los `params` de cada eslabón ya verificado.
