...
SALIDA = RAIZ / "geminis" / "vm-telefono.tar.gz"
...
ARCHIVOS = [
    ...
    "geminis/predicado/vm/src/bin/vectores.rs",
    "geminis/predicado/vm/src/bin/bloque.rs",
    "geminis/predicado/vm/src/bin/mezclas.rs",
...
def main(argumentos: list[str]) -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        with tarfile.open(SALIDA) as tar:
            tar.extractall(tmp)
        vm = Path(tmp) / "geminis" / "predicado" / "vm"
        print(f"\nprobando en {vm} ...")
        r = subprocess.run(
