...
The source of truth is **[Geminis Paper EN.md](Geminis%20Paper%20EN.md)** —~19,500 words, with the
boundaries developed and the record of what was discarded along the way—; the Spanish original is
[Geminis Paper.md](Geminis%20Paper.md), and `Geminis-paper-en.html` is its render, regenerated with
`python render.py en` and never edited by hand.
...
**And if you are here to write code:** [`ROADMAP-EN.md`](ROADMAP-EN.md) has the glossary of the concepts,
the module structure and the phases with their pass criteria.

And every number here has its measurement, with a `RESULTADOS-EN.md` and scripts:

| directory | what it answers |
|---|---|
| [`test1-transicion/`](test1-transicion/RESULTADOS-EN.md) | whether the mechanism has a customer outside — the cases of §7 |
| [`test2-interprete/`](test2-interprete/RESULTADOS-EN.md) | the interpreter's budget on real hardware, with the benchmark package |
| [`test4-ventana-k/`](test4-ventana-k/RESULTADOS-EN.md) | the self-payment attack, and the window that turned out to be empty |
| [`cola-impugnaciones/`](cola-impugnaciones/RESULTADOS-EN.md) | whether the queue of §5.3 saturates — where the ten PoD nodes come from |
| [`expiracion-estado/`](expiracion-estado/RESULTADOS-EN.md) | how much state is generated and what being able to revive it costs |
| [`amortizacion-mint/`](amortizacion-mint/RESULTADOS-EN.md) | why the rate cannot go down by depositing more |
| [`parametros-mint/`](parametros-mint/RESULTADOS-EN.md) | the parameters of §6.4, and which of them is really a decision |
| [`presupuesto-nodo/`](presupuesto-nodo/RESULTADOS-EN.md) | how much an entry occupies, and where `θ*` and `L_max` come from |
...
> else. The measurement is in `geminis/predicado/RESULTADOS-EN.md`.
