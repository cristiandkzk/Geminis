...
de verdad es **[Geminis Paper.es.md](Geminis%20Paper.es.md)** —~19.500 palabras, con las fronteras
desarrolladas y el registro de qué se descartó en el camino—; `Geminis-paper.es.html` es su render
y se regenera con `python render.py es`, no se edita a mano.
...
**Y si venís a escribir código:** [`ROADMAP.es.md`](ROADMAP.es.md) tiene el glosario de los conceptos,
la estructura de módulos y las fases con sus criterios de aprobado.

Y cada número de acá tiene su medición, con `RESULTS.es.md` y scripts:

| directorio | qué contesta |
|---|---|
| [`test1-transicion/`](test1-transicion/RESULTS.es.md) | si el mecanismo tiene cliente afuera — los casos de §7 |
| [`test2-interprete/`](test2-interprete/RESULTS.es.md) | el presupuesto del intérprete en hardware real, con el paquete del benchmark |
| [`test4-ventana-k/`](test4-ventana-k/RESULTS.es.md) | el ataque de auto-pago, y la ventana que resultó vacía |
| [`cola-impugnaciones/`](cola-impugnaciones/RESULTS.es.md) | si la cola de §5.3 satura — de dónde salen los diez nodos PoD |
| [`expiracion-estado/`](expiracion-estado/RESULTS.es.md) | cuánto estado se genera y qué cuesta poder revivirlo |
| [`amortizacion-mint/`](amortizacion-mint/RESULTS.es.md) | por qué la tasa no puede bajar por depositar más |
| [`parametros-mint/`](parametros-mint/RESULTS.es.md) | los parámetros de §6.4, y cuál de ellos es realmente una decisión |
| [`presupuesto-nodo/`](presupuesto-nodo/RESULTS.es.md) | cuánto ocupa una entrada, y de dónde salen `θ*` y `L_max` |
...
> todo lo demás. La medición está en `geminis/predicado/RESULTS.es.md`.
